...
interface Stroke {
  ...
  cornerPts?:  { x: number; y: number }[]; // polygon vertices for triangle/parallelogram
}
...
/** Circularity metric: ~1.0 for circle, ~0.78 for square, lower for elongated shapes */
...
/** Interior angle at b in the chain a→b→c (0..π) */
...
/** Subsample + RDP simplification with adaptive epsilon */
...
/** For a closed shape, remove the duplicate close-point */
...
/** Diamond: each corner should be extremal in ONE axis only (N/E/S/W) */
function isDiamond(corners: Point[]) {
  ...
  for (const p of corners) {
    ...
    if (nearLR !== nearTB) score++; // extremal in exactly one axis
  }
  return score >= corners.length - 1;
}

/** Parallelogram: 4 corners, non-right angles, but one pair of parallel sides */
function isParallelogram(corners: Point[]) {
  if (corners.length < 4) return false;
  // If any angle is ~90° → rectangle, not parallelogram
  const n = 4;
  for (let i = 0; i < n; i++) {
    const ang = interiorAngle(corners[(i+n-1)%n], corners[i], corners[(i+1)%n]) * (180/Math.PI);
    if (Math.abs(ang - 90) < 18) return false;
  }
  // Check one pair of opposite sides is parallel (dot product of direction vectors ≈ 1)
...
/** Arrow: open stroke that doubles back near the tip (V-shape at end) */
function isArrow(pts: Point[], simplified: Point[]) {
  if (simplified.length < 3) return false;
  // Last two simplified segments should form a V angle < 60°
...
function detectShape(pts: Point[]): {
  ...
  // ── Closed: corner-based classification ───────────────────────────────────
  const corners = closedCorners(sim);
  const n = corners.length;
  const circ = circularity(pts);

  // Circle: high circularity AND few corners from simplification
  // Use tight threshold to avoid mis-classifying squares as circles
  if (circ > 0.70 && n <= 5) {
  ...
  if (n >= 4 && n <= 6) {
    ...
    // Diamond first (before rectangle) — same corner count, different orientation
    if (isDiamond(c4))
      return { shape: "diamond", data: { cx, cy, hw: bw / 2, hh: bh / 2 } };

    // Parallelogram: non-right angles
    if (isParallelogram(c4))
      return {
        shape: "parallelogram",
        data: {},
        cornerPts: c4.map(p => ({ x: p.x, y: p.y })),
      };

    // Cylinder: tall rectangle (h ≥ 1.5 × w) — drawn as DB symbol
    if (bh >= bw * 1.5)
  ...
  // Many corners but decent circularity → circle (pentagon/hexagon drawn roughly)
  if (circ > 0.55) {
    const radii = pts.map(p => Math.hypot(p.x - cx, p.y - cy));
    const avgR  = radii.reduce((s, r) => s + r, 0) / radii.length;
    return { shape: "circle", data: { cx, cy, r: avgR } };
...
function renderShape(ctx: CanvasRenderingContext2D, s: Stroke) {
  ...
  switch (s.shape) {
    ...
    case "diamond": {
      // ◇ decision shape — drawn from N/E/S/W extremes
      const { cx, cy, hw, hh } = d;
      ctx.moveTo(cx,      cy - hh);
      ctx.lineTo(cx + hw, cy);
      ctx.lineTo(cx,      cy + hh);
    ...
    case "cylinder": {
      // ⬭ database symbol — rect body + top/bottom ellipses
      ...
      ctx.stroke();
      // bottom ellipse (open top arc only)
      ctx.beginPath();
...
function renderStroke(ctx: CanvasRenderingContext2D, s: Stroke) {
  ...
  // ── natural pressure-sensitive stroke (Catmull-Rom midpoints) ────────────
  if (pts.length === 1) {
...
// ── Letter size aspect ratio: 8.5 × 11 inches ────────────────────────────────
const LETTER_RATIO = 11 / 8.5; // height / width ≈ 1.294
...
export default function NotebookPage() {
  ...
  const [canvasSize, setCanvasSize] = useState({ w: 816, h: 1056 }); // letter @ 96dpi
  ...
  const {
    ...
    onLimitReached: () => {
      // Redirect to pricing if plan limit hit
      window.location.href = "/pricing?reason=pages";
    },
  });
  ...
  // Load theme from account profile
  useEffect(() => {
    if (!isAuth) return;
    fetch("/api/billing/plans").then(r => r.json()).then(() => {
      // theme is applied via Supabase profile on account page
      // here we could fetch it but keep it optional
    });
  }, [isAuth]);

  // ── Mutable refs (avoid stale closure in event handlers) ──────────────────
  ...
  // Load page strokes from server when auth & page changes
  useEffect(() => {
    if (!isAuth) return;
    loadPageFromServer(pageIdx + 1).then(strokes => {
      if (strokes === null) return; // not yet available
  ...
  // ── Pointer events (attached once, read state from refs) ──────────────────
  useEffect(() => {
    ...
    const onUp = (e: PointerEvent) => {
      ...
      // Clear overlay, redraw main
      overlay.getContext("2d")?.clearRect(0, 0, overlay.width, overlay.height);
      redraw();

      // Auto-save to Supabase (debounced 1.5s)
      savePageToServer(pageIdxRef.current + 1, next[idx].strokes as unknown[]);
    };
  ...
  }, [redraw]); // attach once — canvas never unmounts, reads state via refs
...
<script>
  ...
  // ── Style helpers (inline, no inner components) ────────────────────────────
  ...
  // ── Shared JSX blocks (inlined as variables, not components) ──────────────
  ...
  const toolbarJSX = (
    <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", alignItems: "center", padding: "10px 0" }}>
      ...
      {tabletDetected && (
        <span style={{ fontSize: "11px", color: "#22c55e" }}>✒️ Tablet detectada</span>
      )}
      ...
      {/* Auth gate */}
      {!isAuth && (
        <Link href="/auth/login?next=/tools/notebook" style={{
  ...
  // ── Single tree — containerRef only changes CSS (fixed vs normal) ────────────
  return (
    <div style={{ minHeight: "100vh", background: "var(--background)" }}>

      {/* ── Page header (only visible when not fullscreen) ── */}
      {!isFullscreen && (
      ...
      {/* ── Sticky toolbar (full width, only when not fullscreen) ── */}
      {!isFullscreen && (
        <div style={{
      ...
      {/*
        containerRef — same node always in the DOM.
        Normal: no position set (flows naturally, full width).
        Fullscreen: position:fixed inset:0 z-index:9999.
        Canvas children are NEVER unmounted.
      */}
      <div
        ref={containerRef}
        style={isFullscreen ? {
      ...
      >
        {/* Toolbar inside the fixed overlay — only in fullscreen */}
        {isFullscreen && (
          <div style={{
            width: "100%",
        ...
        {/*
          Canvas wrapper.
          Normal mode: full-width, aspect-ratio carta (8.5×11).
          Fullscreen: flex:1 to fill remaining height.
        */}
        <div
          ref={wrapperRef}
          style={isFullscreen ? {
            ...
            width: "100%",
            maxWidth: 900,
            aspectRatio: `${1 / LETTER_RATIO}`,  // 8.5/11 ≈ 0.773
            borderRadius: 10,
            overflow: "hidden",
        ...
        >
          ...
          {shapeMode && (
            <div style={{
              ...
              pointerEvents: "none",
            }}>⬡ Reconocimiento de figuras activo</div>
          )}
        </div>
      </div>
